...
double unghi = (3.14 / 180.0) * -30;

double cabinetMatrix[16] = {
    1, 0, 0, 0,
    0, 1, 0, 0,
    -cos(unghi) / 2.0, sin(unghi) / 2.0, 1, 0,
    0, 0, 0, 1
};
...
void Display()
{
    ...
    case 'w':
        ...
        break;
    case 'e':
        // first image; proiectie paralela ortografica verticala ((Deasupra))^
        proiectieParalela('e');
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
    ...
    case 'r':
        // second image
        proiectieParalela('r'); // proiectiile
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        glPushMatrix();
    ...
    case 't':
        // third image
        proiectieParalela('t'); // proiectiile
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        glPushMatrix();
        glMultMatrixd(cabinetMatrix);
        glScalef(0.5, 0.5, 0.5); 
        glTranslated(-lat, -lat, -lat * 2.1);
...
void proiectieParalela(unsigned char c) {
    ...
    case 'e':
        ...
        break;
    case 'r':
        glFrustum(-5, 5, -5, 5, 1.5, 14);
        glTranslatef(-lat - 4, -lat - 4, -lat - 4);
        break;
    case 't':
        glOrtho(-2, 4, -3, 3, -1, 10);
        break;
    default:
        break;
    }
